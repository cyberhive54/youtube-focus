...
|---|---|---|---|
| 3.4.2 | 2026-09-15 | Removed card allowlists under the chevron (managed inside Add/Edit modal) and added dedicated schedule analytics: 4-stat summary grid (Current/Best Streaks, Total Focus Time, Adherence Rate, Breaks Taken), 7-Day Adherence Bar Graph, and Run Sessions Log with inline scrolling box. | JS syntax checks; test_v342_schedule_stats passed; policy 136/136 |
| 3.4.1 | 2026-09-15 | Fixed schedule breaks validation and silent save failure when editing nothing: automatically initializes smart, valid defaults matching duration and strictness upon enabling breaks; automatically auto-opens collapsed advanced details and surfaces error in modal error banner if validation fails; added max-height and scrolling to modal dialogs. | JS syntax checks; unit test passed; policy 136/136 |
| 3.4.0 | 2026-09-15 | Enforced 30-minute minimum schedule duration; Advanced Collapsible Breaks Configuration (break time 2.5%–10% strict / 5%–20% non-strict; cooldown 5%–50% strict / 5%–75% non-strict; max single break 50% strict / 75% non-strict); YouTube on-page block screen 'Take a break' choice card; on-page floating break indicator with live timer and early refund action; 1-minute expiration warning toast and sound chime. | JS syntax checks; test_v340_breaks_advanced passed; policy 136/136 |
| 3.3.0 | 2026-09-15 | In-modal Study Mode allowlists (channels & videos) directly inside Add/Edit Schedule modal; Schedule Breaks System with strict mode rules (max break time $\le 20\%$ non-strict / $\le 10\%$ strict; break count 1–20 non-strict / 1–10 strict; no per-break time limit; strict lockout during pre-timed lock and active schedule); popup break activation and live countdown card; normal browsing under global rules during breaks; daily reset. | JS syntax checks; test_v330_breaks passed; policy 136/136 |
